...
final clientAccountRepositoryProvider =
    FutureProvider<ClientAccountRepository>((ref) async {
...
final clientSummaryProvider =
    FutureProvider.family<ClientAccountSummary, int>((ref, clientId) async {
  final repo = await ref.watch(clientAccountRepositoryProvider.future);
  return repo.clientSummary(clientId);
});

final clientLedgerProvider = FutureProvider.family<
    List<ClientLedgerLine>,
    ({int clientId, DateTime? from, DateTime? to})>((ref, arg) async {
  final repo = await ref.watch(clientAccountRepositoryProvider.future);
  return repo.clientLedger(arg.clientId, from: arg.from, to: arg.to);
});

final clientPaymentsProvider =
    FutureProvider.family<List<ClientPayment>, int?>((ref, clientId) async {
  final repo = await ref.watch(clientAccountRepositoryProvider.future);
  return repo.listPayments(clientId: clientId);
});

final clientChargesProvider =
    FutureProvider.family<List<ClientCharge>, int?>((ref, clientId) async {
  final repo = await ref.watch(clientAccountRepositoryProvider.future);
...
final chargesWithOpenAmountProvider = FutureProvider.family<
    List<({ClientCharge charge, int openMinor})>,
    int>((ref, clientId) async {
  final repo = await ref.watch(clientAccountRepositoryProvider.future);
...
final chargesRegisterProvider = FutureProvider<List<ClientCharge>>((
  ref,
) async {
  final repo = await ref.watch(clientAccountRepositoryProvider.future);
...
final statementPreviewProvider = FutureProvider.family<
    StatementPreview,
    ({int clientId, DateTime from, DateTime to})>((ref, arg) async {
  final repo = await ref.watch(clientAccountRepositoryProvider.future);
...
final paymentDetailProvider = FutureProvider.family<
    ({ClientPayment? payment, Client? client}), int>((ref, paymentId) async {
  final repo = await ref.watch(clientAccountRepositoryProvider.future);
  final payment = await repo.getPayment(paymentId);
  final client =
      payment != null ? await repo.getClient(payment.clientId) : null;
